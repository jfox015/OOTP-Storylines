<div class="admin-box">
	...
	<table class="table table-striped">
		...
		<?php if (isset($tokens) && is_array($tokens) && count($tokens)) : ?>
		<tfoot>
		<tr>
			<td colspan="6">
		...
		<?php if (isset($tokens) && is_array($tokens) && count($tokens)) : ?>
			...
			<tr>
				<td>
					<input type="checkbox" name="checked[]" value="<?php echo $token->id ?>" />
				</td>
				<td><?php echo $token->id ?></td>
				<td>
					<a href="<?php echo site_url(SITE_AREA .'/custom/storylines/tokens/edit/'. $token->id); ?>"><?php echo $token->slug; ?></a>
				</td>
				<td><?php echo $token->name ?></td>
				<td><?php echo $token->category_name ?></td>
			</tr>
				<?php endforeach; ?>
			<?php else: ?>
		<tr>
